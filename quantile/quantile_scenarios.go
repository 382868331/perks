...
func QuantileDuplicateSampleStability(in []int) []int {
	seen := map[int]bool{}
	out := []int{}
	for _, v := range in {
		if !seen[v] {
...
func QuantileObjectiveBooleanOption(s string) (bool, error) {
...
func QuantileInvariantGrowthOverflow(base int, attempt int, capPow int) int {
	if attempt < 0 {
		return 0
	}
	if attempt > capPow {
...
func QuantileSummaryCloneIsolation(in map[string]map[string]int) map[string]map[string]int {
...
func QuantileReverseMetricRunes(s string) string {
	r := []rune(s)
...
func QuantileWindowInclusiveTail(in []int, size int) [][]int {
...
func QuantileEmptySummaryRendering(parts []string, sep string) string {
	if len(parts) == 0 {
		return ""
	}
...
func QuantileParallelInsertCount(n int) int {
	var mu sync.Mutex
	v := 0
...
func QuantileCanceledStreamDrain(ctx context.Context, n int) int {
	done := 0
	for done < n {
		select {
		case <-ctx.Done():
...
func QuantileTupleDeleteIterator(in []int) []int {
